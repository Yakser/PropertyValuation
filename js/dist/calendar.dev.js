...
function createMonthTable(data, option) {
  ...
  tr = document.createElement("tr"); //create 1st row for the day letters
  ...
  table.appendChild(tr); //create 2nd row for dates

  tr = document.createElement("tr"); //blank td
  ...
  table.appendChild(tr); //create remaining rows
...
function drawCalendarMonthTable(data, option) {
  var table, div, container, elem; //get table

  table = createMonthTable(data, option); //calendar container

  container = document.createElement("div");
  container.setAttribute("class", "month-container"); //-------------------------- Header ------------------
  //header div

  div = document.createElement("div");
  div.setAttribute("class", "header");
  div.setAttribute("data-option", JSON.stringify(option)); //prev button

  if (option.prevnextbutton === "show") {
    elem = document.createElement("span");
    elem.setAttribute("class", "prev-next-btn prev-btn");
    ...
    elem.style.background = "url('../img/right-arrow.svg')";
    elem.style.backgroundSize = "cover";
    elem.style.transform = "rotate(-180deg)";
    elem.style.width = "1.5rem";
    elem.style.height = "1.5rem";
    elem.innerHTML = ""; //add prev button span to header div
  ...
  elem = document.createElement("span");
  elem.setAttribute("class", "span-month-year");
  ...
  div.appendChild(elem); //next button

  if (option.prevnextbutton === "show") {
    elem = document.createElement("span");
    elem.setAttribute("class", "prev-next-btn next-btn");
    elem.setAttribute("data-date", option.date);
    elem.setAttribute("data-month", option.month);
    elem.setAttribute("data-year", option.year);
    ...
    elem.style.backgroundSize = "cover";
    elem.style.width = "1.5rem";
    elem.style.height = "1.5rem"; //add prev button span to header div

    div.appendChild(elem);
  } //add header div to container


  container.appendChild(div); //-------------------------- Body ------------------
  //body div

  div = document.createElement("div");
  div.setAttribute("class", "body");
  div.appendChild(table); //add body div to container div
...
function drawCalendarDay(data, option) {
  var div, container, elem; //calendar container

  container = document.createElement("div");
  container.setAttribute("class", "day-container"); //-------------------------- Header ------------------
  //header div

  div = document.createElement("div");
  div.setAttribute("class", "header"); //day span

  elem = document.createElement("span");
  elem.setAttribute("class", "span-day");
  ...
  div.appendChild(elem); //add header div to container

  container.appendChild(div); //-------------------------- Body ------------------
  //body div

  div = document.createElement("div");
  div.setAttribute("class", "body"); //date span

  elem = document.createElement("span");
  elem.setAttribute("class", "span-date");
  elem.innerHTML = data.date; //add date span to body div

  div.appendChild(elem); //add body div to container

  container.appendChild(div); //-------------------------- Footer ------------------
  //footer div

  div = document.createElement("div");
  div.setAttribute("class", "footer"); //month span
  ...
  div.appendChild(elem); //add footer div to container
...
function getCalendar(year, month, date) {
  ...
  result.today.year = dateString[3]; //get month-year first day
  ...
  result.monthNameFull = monthName.full[idx]; //get total days for the month-year

  dateObj.setFullYear(year);
  dateObj.setMonth(month + 1);
  dateObj.setDate(0);
  result.totaldays = dateObj.getDate(); //get month-year targeted date
...
function onClick() {
  document.body.onclick = function (e) {
    ...
    e = event || e;
    var targetDomObject = e.target,
        date,
        month,
        year,
        btn,
        option,
        dateObj; //prev-next button click
    //extra checks to make sure object exists and contains the class of interest

    if (targetDomObject && targetDomObject.classList && targetDomObject.classList.contains("prev-next-btn") || targetDomObject.classList.contains("prev-next-btn-ico")) {
    ...
    if (targetDomObject && targetDomObject.classList && targetDomObject.classList.contains("span-month-year")) {
      option = JSON.parse(targetDomObject.parentElement.getAttribute("data-option"));
      dateObj = new Date();
...
function drawCalendar(option) {
  ...
  i,
      len,
      elemArr; //find target element by
  ...
  targetElem = option.target.substring(1); //get calendar HTML

  switch (option.type) {
    case "day":
      //get calendar detail
      calendar = getCalendar(option.year, option.month, option.date); //get calendar html
